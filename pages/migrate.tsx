...
function Migrate() {
  ...
  const {handleTx, txStatus: harvestStatus, txInProgress: harvIP, txError} = useContractTransaction(1)
  ...
  if (!approved) {
    return (<Fragment>
        ...
        <main className={styles.main}>
          <h1>Migrate</h1>
          <p>Before you can migrate {"you'll"} need to approve the new V2 Contract</p>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.migrate}>
          {tokenIds.map((_, index) =>
            <input className={styles.input} id={index.toString()} key={index} placeholder={'Token ID'}
                   onChange={onChange}/>
          )}
          <div>
